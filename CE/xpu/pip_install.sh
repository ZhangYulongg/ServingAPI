...
function py_requirements () {
  ...
  if [ $1 == 36 ]; then
      ...
      $py_version -m pip install --ignore-installed paddlepaddle-2.2.0-cp36-cp36m-linux_x86_64.whl -i https://mirror.baidu.com/pypi/simple
  elif [ $1 == 37 ]; then
      ...
      $py_version -m pip install --ignore-installed paddlepaddle-2.2.0-cp37-cp37m-linux_x86_64.whl -i https://mirror.baidu.com/pypi/simple
  else
      echo -e "${RED_COLOR}Error py version$1${RES}"
      exit
  fi
