...
${py_version} ${client_py} --thread 10 --batch_size 1 > profile 2>&1
echo -e "import psutil\nimport time\nwhile True:\n\tcpu_res = psutil.cpu_percent()\n\twith open('cpu.txt', 'a+') as f:\n\t\tf.write(f'{cpu_res}\\\n')\n\ttime.sleep(0.1)" > cpu.py
for thread_num in 1 5 10 15 20 25 50 100
do
for batch_size in 1
do
    echo "#----${model_name} thread num: $thread_num batch size: $batch_size mode:brpc ----" >> profile_log_resnet_v2_50
    ...
    ${py_version} cpu.py &
    gpu_memory_pid=$!
    ${py_version} ${client_py} --thread $thread_num --batch_size $batch_size > profile 2>&1
