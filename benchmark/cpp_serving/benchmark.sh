...
${py_version} benchmark.py --thread 10 --batch_size 1 --model resnet_v2_50_imagenet_client/serving_client_conf.prototxt --request rpc > profile 2>&1
echo -e "import psutil\nimport time\nwhile True:\n\tcpu_res = psutil.cpu_percent()\n\twith open('cpu.txt', 'a+') as f:\n\t\tf.write(f'{cpu_res}\\\n')\n\ttime.sleep(0.1)" > cpu.py
for thread_num in 1 2 4 6 8 12 16
do
for batch_size in 1
do
    echo "#----ResNet_v2_50 thread num: $thread_num batch size: $batch_size mode:brpc ----" >> profile_log_resnet_v2_50
    ...
    ${py_version} cpu.py &
    gpu_memory_pid=$!
    ${py_version} benchmark.py --thread $thread_num --batch_size $batch_size --model resnet_v2_50_imagenet_client/serving_client_conf.prototxt --request rpc > profile 2>&1
    ...
    ${py_version} cpu_utilization.py >> profile_log_resnet_v2_50
    awk 'BEGIN {max = 0} {if(NR>1){if ($1 > max) max=$1}} END {print "CPU_UTILIZATION_MAX:", max}' cpu.txt >> profile_log_resnet_v2_50
    grep -av '^0 %' gpu_utilization.log > gpu_utilization.log.tmp
    awk 'BEGIN {max = 0} {if(NR>1){if ($model_name > max) max=$model_name}} END {print "MAX_GPU_MEMORY:", max}' gpu_memory_use.log >> profile_log_resnet_v2_50
    ...
    ${py_version} ../util/show_profile.py profile $thread_num >> profile_log_resnet_v2_50
