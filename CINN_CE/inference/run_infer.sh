...
function main() {
    ...
    ${py_version} test_resnet18.py $PWD/ResNet18 ON 2>&1 | tee -a ${log_dir}/ResNet18.log
    ${py_version} test_mobilenetv2.py $PWD/MobileNetV2 ON 2>&1 | tee -a ${log_dir}/MobileNetV2.log
    ${py_version} test_efficientnet.py $PWD/EfficientNet ON 2>&1 | tee -a ${log_dir}/EfficientNet.log
    ${py_version} test_mobilenetv1.py $PWD/MobilenetV1 ON 2>&1 | tee -a ${log_dir}/MobilenetV1.log
    ${py_version} test_resnet50.py $PWD/ResNet50 ON 2>&1 | tee -a ${log_dir}/ResNet50.log
    ${py_version} test_squeezenet.py $PWD/SqueezeNet ON 2>&1 | tee -a ${log_dir}/SqueezeNet.log
    ${py_version} test_facedet.py $PWD/FaceDet ON 2>&1 | tee -a ${log_dir}/FaceDet.log
}
